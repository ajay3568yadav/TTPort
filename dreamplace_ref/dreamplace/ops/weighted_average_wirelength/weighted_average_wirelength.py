...
class WeightedAverageWirelengthFunction(Function):
    ...
    def backward(ctx, grad_pos):
        ...
        pin_mask_bool = ctx.pin_mask.to(torch.bool) if ctx.pin_mask.dtype != torch.bool else ctx.pin_mask
        output[:output.numel() // 2].masked_fill_(pin_mask_bool, 0.0)
        output[output.numel() // 2:].masked_fill_(pin_mask_bool, 0.0)
        if grad_pos.is_cuda:
            torch.cuda.synchronize()
...
class WeightedAverageWirelengthAtomicFunction(Function):
    ...
    def backward(ctx, grad_pos):
        ...
        output = func(grad_pos, ctx.pos, ctx.exp_xy.view([-1]),
                      ...
                      ctx.net_mask, ctx.inv_gamma)
        pin_mask_bool = ctx.pin_mask.to(torch.bool) if ctx.pin_mask.dtype != torch.bool else ctx.pin_mask
        output[:int(output.numel() // 2)].masked_fill_(pin_mask_bool, 0.0)
        output[int(output.numel() // 2):].masked_fill_(pin_mask_bool, 0.0)
        if grad_pos.is_cuda:
            torch.cuda.synchronize()
        logger.debug("wirelength backward %.3f ms" %
...
class WeightedAverageWirelength(nn.Module):
    ...
    def __init__(self,
        ...
        @param algorithm must be net-by-net | atomic | merged | merged_tt
        @param tt_device open ttnn device when algorithm is merged_tt (set params._tt_wl_device)
        @param tt_backend ignored (merged_tt uses TTNN ``ttnn_wa_opt`` only)
        """
        ...
        if algorithm in ['net-by-net', 'merged', 'merged_tt']:
        ...
        if algorithm == 'merged_tt':
            assert tt_device is not None, "merged_tt requires tt_device (open ttnn device on params._tt_wl_device)"
        ...
        self.tt_backend = tt_backend  # unused; merged_tt is TTNN-only
    ...
    def forward(self, pos):
        ...
        elif self.algorithm == 'merged_tt':
            from dreamplace.ops.weighted_average_wirelength.wa_merged_tt import (
                get_merged_tt_function_class,
            )

            MF = get_merged_tt_function_class(self.tt_device)
            return MF.apply(
                pos,
                self.flat_netpin,
                self.netpin_start,
